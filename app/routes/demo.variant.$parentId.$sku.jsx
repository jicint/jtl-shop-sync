...
export const loader = async ({ params }) => {
  ...
  for (const category of categories) {
    for (const product of category.products || []) {
      ...
      const variant = (product.child_variants || []).find((entry) => entry.sku === params.sku);
      if (!variant) continue;
      ...
      return {
        ...
        gallery: Array.isArray(variant.child_gallery) ? variant.child_gallery : [],
      };
    }
  }
...
function buildVariantPreview(productType, title, values, viewLabel = "Front") {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="900" height="900" viewBox="0 0 360 360">
      ...
      ${silhouettes[kind]}
      <rect x="24" y="24" width="118" height="34" rx="17" fill="#ffffff" fill-opacity=".94"/>
      <text x="83" y="46" font-family="Arial, sans-serif" font-size="18" font-weight="700" text-anchor="middle" fill="#0f172a">${label}</text>
      <rect x="228" y="24" width="108" height="34" rx="17" fill="#0f172a" fill-opacity=".84"/>
      <text x="282" y="46" font-family="Arial, sans-serif" font-size="16" font-weight="700" text-anchor="middle" fill="#f8fafc">${viewLabel}</text>
    </svg>
  `;
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", display: "grid", gap: "1.5rem" }}>
        ...
        <div
          style={{
            ...
            display: "grid",
            gap: "1.5rem",
            gridTemplateColumns: "minmax(320px, 1.1fr) minmax(0, 0.9fr)",
          }}
        >
            ...
            <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(3, minmax(0, 1fr))" }}>
              {gallery.slice(1, 4).map((image, index) => (
                <img
                  ...
                  style={{ width: "100%", borderRadius: "14px", display: "block", background: "#e2e8f0" }}
                />
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <h1 style={{ margin: "0.45rem 0 0 0", color: "#0f172a" }}>
              {variant.title} - {label}
            </h1>
            <div style={{ marginTop: "0.9rem", color: "#0f766e", fontWeight: 700, fontSize: "1.25rem" }}>
              ${Number(variant.price || 0).toFixed(2)}
            </div>
            <p style={{ margin: "0.85rem 0 0 0", color: "#475569", lineHeight: 1.7 }}>{variant.description}</p>

            <div style={{ marginTop: "1.2rem", display: "grid", gap: "0.7rem", color: "#334155" }}>
